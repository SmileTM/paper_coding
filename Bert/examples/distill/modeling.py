...
class BertModel(tf.keras.layers.Layer):
    ...
    def call(self, inputs, mode="bert"):
        ...
        if mode == 'distill':
            attns_output = self.encoder((input_tensor, attention_mask), return_all_layers=True)
            sequence_output = attns_output[-1]
            ...
            return input_tensor, attns_output, pooled_output
